"""
Oracle database backend for Django using pyorapool as connection pool

Copyright 2007 Stefan Bethge and others (see django)
This file is licensed under the BSD License as it is derived from the django source
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def _cursor(self, settings):
        if not self._valid_connection():
            proxy = Pyro.core.getProxyForURI(settings.PYRO_URI_DJANGO)
            self.connection = proxy.connect()
