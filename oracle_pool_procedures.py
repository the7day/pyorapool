...
# Copyright (C) 2007-2010 Music Pictures Ltd
...
import datetime
import util.uuid as uuid #for debugging

"""

Note!! If you get an error like below:
 look into the error.log and see the full traceback
...
class package:
    ...
    def fn_stored_function(self, cx_connection, parameter):
        ...
        cursor.close()
        #connection will be closed from the pool if necessary, so leave it open
        return retVal

    def pr_stored_procedure(self, cx_connection, parameter_in=None):
        cx_connection = self.cx_connect()
        cursor = cx_connection.internal_cursor()
